...
test('release metadata is v0.9.2 and remains hidden from the page',()=>{
  const pkg=JSON.parse(readFileSync(new URL('../package.json',import.meta.url),'utf8'));
  const html=readFileSync(new URL('../dist/index.html',import.meta.url),'utf8');
  assert.equal(pkg.version,'0.9.2');
  assert.match(html,/style\.css\?v=0\.9\.2/);
  assert.match(html,/app\.js\?v=0\.9\.2/);
...
test('status cards distinguish no alerts, active alerts, unknown data and fetch failures',async()=>{
  ...
  runInContext('updateWarning(warning,stamp)',context);
  assert.equal(node('#weatherStatus').textContent,'警報発表中');
  assert.equal(node('#weatherDot').className,'dot warn');
  assert.equal(node('#weatherDetail').textContent,'レベル3相当｜大雨警報');
  assert.match(node('#weatherTime').textContent,/確認 .*｜発表 /);
  context.warning={state:'ok',warnings:[{code:'03',status:'発表'},{code:'29',status:'継続'}],reportDatetime:stamp};
  runInContext('updateWarning(warning,stamp)',context);
  assert.equal(node('#weatherStatus').textContent,'警報発表中');
  assert.equal(node('#weatherDetail').textContent,'レベル3相当｜大雨警報 ／ レベル2相当｜土砂災害注意報');
  context.warning={state:'ok',warnings:[{code:'43',status:'発表'}],reportDatetime:stamp};
  runInContext('updateWarning(warning,stamp)',context);assert.equal(node('#weatherDot').className,'dot lv4');
  context.warning={state:'ok',warnings:[{code:'33',status:'発表'}],reportDatetime:stamp};
  runInContext('updateWarning(warning,stamp)',context);assert.equal(node('#weatherDot').className,'dot lv5');
  context.warning={state:'ok',warnings:[{code:'99',status:'発表'}],reportDatetime:stamp};
  runInContext('updateWarning(warning,stamp)',context);assert.notEqual(node('#weatherDot').className,'dot ok');assert.match(node('#weatherDetail').textContent,/未対応/);
  ...
  for(const [highest,label] of [[3,'高齢者等避難'],[4,'避難指示'],[5,'緊急安全確保']]){
    payload={state:'active',highest,summary:'対象地域',retrievedAt:stamp,reportDatetime:stamp};await runInContext('loadEvacuation()',context);
    assert.equal(node('#evacStatus').textContent,label+' 発令中');
    assert.equal(node('#evacDot').className,'dot '+(highest===5?'lv5':highest===4?'lv4':'warn'));
    assert.match(node('#evacTime').textContent,/確認 .*｜発表 /);
  }
...
test('current frontend has ordered alerts, Tokyo live cameras, profile SNS cards and visual link cards',()=>{
  ...
  assert.match(html,/<span>避難場所・避難所<\/span>/);assert.match(html,/<span>水害に備えて<\/span>/);
  assert.doesNotMatch(html,/荒川区災害情報サイト　避難場所・避難所|荒川区 &gt; 防災 &gt; 水害に備えて/);
  ...
  assert.match(html,/style\.css\?v=0\.9\.2/);assert.match(html,/v08-ui\.css\?v=0\.9\.2/);
